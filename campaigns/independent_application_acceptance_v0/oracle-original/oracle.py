"""Frozen copied-byte oracle. Standard library only; imports no product module."""
import hashlib
import io
import json
...
def unpack(raw,canonical=False,maximum=64*1024*1024):
    ...
    with zipfile.ZipFile(io.BytesIO(raw)) as z:
        ...
        for i in infos:
            safe(i.filename);need(i.filename.casefold() not in folded,'duplicate_path');folded.add(i.filename.casefold())
...
def interaction_check(d,i,members):
    ...
    text(d['name']);text(d['description']);safe(d['entrypoint']);need(d['entrypoint'].endswith('.py') and d['entrypoint'] in members,'python_entrypoint')
...
def candidate(raw):
    ...
    app=unpack(parts[NAMES[1]]);d=tomllib.loads(app['capability.toml'].decode());i=parse(parts[NAMES[2]])
    ...
    safe(d['entrypoint']);need(d['entrypoint'] in app,'entrypoint')
...
def validate(candidate_bytes,profile_bytes,document_bytes,release):
    ...
    need(doc['identity']==identity and p['application_id']==m['application']['id'],'acceptance_binding')
    h=sha(canon(identity));need(doc['identity_sha256']==h and doc['acceptance_id']=='acc_'+h[:32],'acceptance_identity')
    a=m['application'];need(doc['source']==m['source'] and doc['toolchain']==m['toolchain'],'portable_binding')
    ...
    for row,case in zip(doc['cases'],p['cases']):
        keys(row,['case_id','matched','classification','expected','observed']);need(row['case_id']==case['case_id'] and row['expected']==expected(case,parts),'case_expected')
        ...
        if row['matched']:need(row['observed']==row['expected'] and row['classification']=='CASE_MATCHED','forged_case_match')
        else:
            ...
            need(row['observed']!=row['expected'] or row['classification'] in ['REJECTED_SECRET_BOUNDARY','REJECTED_OUTPUT_LIMIT','REJECTED_APPLICATION_EXIT'],'false_mismatch')
